...
class ItemAdapter(
    ...
    class ItemVH(private val b: ItemItemBinding) : RecyclerView.ViewHolder(b.root) {
        fun bind(item: Item, onEditar: (Item) -> Unit, onAlternar: (Item) -> Unit) {
            ...
            // Usar ícones coloridos de categoria
            val iconRes = when (item.categoria) {
                Categoria.ALIMENTOS -> com.example.projeto_programaomobile_parte1.R.drawable.ic_categoria_alimentos_color
                Categoria.BEBIDAS -> com.example.projeto_programaomobile_parte1.R.drawable.ic_categoria_bebidas_color
                Categoria.HIGIENE -> com.example.projeto_programaomobile_parte1.R.drawable.ic_categoria_higiene_color
                Categoria.LIMPEZA -> com.example.projeto_programaomobile_parte1.R.drawable.ic_categoria_limpeza_color
                Categoria.OUTROS -> com.example.projeto_programaomobile_parte1.R.drawable.ic_categoria_outros_color
            }
            b.imgCategoria.setImageResource(iconRes)
            b.btnMenu.setOnClickListener { onEditar(item) }
        }
    }
}
